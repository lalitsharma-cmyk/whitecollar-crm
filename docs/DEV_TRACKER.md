...
- **DEPLOY (held per Lalit):** now MUCH simpler — only 1 additive migration `20260701130000_buyer_field_history`. Checklist: backup → apply that migration to Neon (schema leads code) → merge `ws-unify-lead-view` → main → `tsc + regression` gate → `npm run push` → verify `/api/health` commit.
