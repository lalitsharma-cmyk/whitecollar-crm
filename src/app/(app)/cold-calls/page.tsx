...
// ❄ COLD DATA — admin imports a CSV/Excel batch and assigns rows to agents.
// Agents see only their assigned rows. When connected and qualified, agent
// taps "Promote to Lead" which flips isColdCall=false → the row moves into
// the main /leads pipeline as the agent's owned lead and disappears here.
//
// Three sub-buckets:
...
export default async function ColdDataPage({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  const [leads, manualCount, bantCount, staleCount, totalCount, unassignedCount, agents, convertedTodayCount] = await Promise.all([
  ...
  return (
    <>
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold">❄ Cold Data</h1>
          <p className="text-xs sm:text-sm text-gray-500">
            {isAdminOrMgr
              ? "Import a batch of prospects, assign to agents. Agents work them, promote connected ones to Leads."
              : "Your assigned outbound prospects. Connect → promote to Lead. Otherwise call once a week, not daily."}
          </p>
