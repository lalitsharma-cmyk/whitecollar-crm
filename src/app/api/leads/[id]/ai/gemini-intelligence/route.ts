...
import { analyzeLeadWithGemini, geminiIntelligenceEnabled, isAiPilotLead, getLatestGeminiIntelligence } from "@/lib/ai-gemini-intelligence";
...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!isAiPilotLead(lead.ownerId)) {
    return NextResponse.json({ error: "Gemini Intelligence pilot is only available for Lalit Sharma's leads." }, { status: 403 });
  }
  ...
  try {
    const { analysisId, result } = await analyzeLeadWithGemini(
      lead as Parameters<typeof analyzeLeadWithGemini>[0],
      me.id,
      body.reanalyze ? "re-analyze" : "manual"
    );
    return NextResponse.json({ analysisId, result });
  } catch (err) {
    return NextResponse.json({ error: err instanceof Error ? err.message : "Gemini analysis failed" }, { status: 500 });
  }
}
...
export async function GET(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  await requireUser();
  const { id } = await params;
  const analysis = await getLatestGeminiIntelligence(id);
