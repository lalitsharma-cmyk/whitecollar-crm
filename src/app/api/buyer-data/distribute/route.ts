...
async function buildPlan(
  ...
  if (mode === "assignN" || mode === "byRegion") {
    ...
    if (mode === "byRegion") {
      ...
      n = await poolCount(regionWhere(region));
    } else {
      n = Math.max(0, Math.floor(Number(body.n) || 0));
    }
    const plan = await planAssignN(agent, n, region);
  ...
  if (mode === "splitEqually") {
    ...
    const plan = await planSplitEqually(agentsList, { region, limit });
...
export async function POST(req: NextRequest) {
  ...
  // Roster the actor may assign to. MANAGER → their subtree only; ADMIN → all active.
  ...
  if (me.role === "MANAGER") {
    ...
    const all = await activeAgents();
    roster = allowed === null ? all : all.filter((a) => allowed.includes(a.id));
  } else {
    roster = await activeAgents();
  }

  const { plan, targetAgentIds, error } = await buildPlan(mode, body, roster);
  ...
  if (phase === "apply") {
    const result = await applyPlan(plan, me.id, { reason: `Distributed by ${me.name}` });
    await audit({
      userId: me.id, action: "buyer.distribute", entity: "BuyerRecord",
      meta: { mode, totalAssigned: result.totalAssigned, perAgent: result.perAgent, targetAgentIds },
      request: reqMeta(req),
    });
...
// Read the daily auto-distribution toggle state (for the panel switch).
