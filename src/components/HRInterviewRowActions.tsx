...
export default function HRInterviewRowActions({
  ...
  async function patch(payload: Record<string, unknown>) {
    ...
    try {
      ...
      if (!r.ok) { const e = await r.json().catch(() => ({})); alert(e.error || "Action failed"); return false; }
      return true;
  ...
  return (
    <div className="flex items-center gap-1 flex-wrap">
      ...
      <button type="button" disabled={busy} onClick={() => setModal("reschedule")}
      ...
      <button type="button" disabled={busy} onClick={() => setModal("result")}
      ...
      {/* ── Reschedule modal ── */}
      {modal === "reschedule" && (
        <Modal title="Reschedule Interview" onClose={() => setModal("none")}>
          ...
          <div className="flex justify-end gap-2 mt-4">
            <button type="button" onClick={() => setModal("none")} className="px-3 py-1.5 text-sm rounded-lg border border-gray-300 dark:border-slate-600">Cancel</button>
            <button type="button" disabled={busy || !newAt} onClick={submitReschedule}
              className="px-3 py-1.5 text-sm rounded-lg bg-purple-600 text-white hover:bg-purple-700 disabled:opacity-50">Reschedule</button>
          </div>
        </Modal>
      )}
      ...
      {/* ── Record-result modal ── */}
      {modal === "result" && (
        <Modal title="Record Interview Result" onClose={() => setModal("none")}>
          ...
          <div className="flex justify-end gap-2 mt-4">
            ...
            <button type="button" disabled={busy} onClick={submitResult}
              className="px-3 py-1.5 text-sm rounded-lg bg-blue-600 text-white hover:bg-blue-700 disabled:opacity-50">Save Result</button>
          </div>
        </Modal>
      )}
    </div>
