...
export async function loadOwnedLead(leadId: string): Promise<
  | { me: Awaited<ReturnType<typeof requireUser>>; lead: { id: string; ownerId: string | null; phone: string | null; name: string; forwardedTeam: string | null; currentStatus: string | null }; error?: undefined }
  | { error: NextResponse; me?: undefined; lead?: undefined }
> {
  const me = await requireUser();
  const lead = await prisma.lead.findUnique({
    where: { id: leadId },
    // currentStatus is needed by the inline-update route to enforce the
    // "terminal lead carries no follow-up" invariant when only followupDate is
    // edited (currentStatus absent from the PATCH body).
    select: { id: true, ownerId: true, phone: true, name: true, forwardedTeam: true, currentStatus: true },
  });
