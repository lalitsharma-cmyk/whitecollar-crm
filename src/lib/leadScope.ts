...
//
// Rules:
//   ADMIN, MANAGER  → see and act on every lead
...
export function leadScopeWhere(me: ScopedUser): { ownerId?: string } {
  if (me.role === "AGENT") return { ownerId: me.id };
  return {};
}

/** True if the user is allowed to access this specific lead. */
export function canTouchLead(me: ScopedUser, lead: { ownerId: string | null }): boolean {
  if (me.role === "ADMIN" || me.role === "MANAGER") return true;
  return lead.ownerId === me.id;
}
