...
// Default policy (Lalit 2026-06-28): 3 approved devices — laptop + mobile + 1 spare —
// adjustable per user via deviceLimitExtra (total = 3 + extra, floored at 1 so an admin
// can lock a user to a SINGLE device; extra ranges −2…2 → total 1…5). This is the CAP
// on APPROVED devices (enforced at approval time), NOT an auto-approve threshold.
// Exported so the admin approve action shares the rule.
